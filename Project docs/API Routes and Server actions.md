...
From what we know for now those are:
...
So, I think you’ve got the idea of what’s happening with both of them. Remember that if you want to define actions for a specific feature, include them in the `features/[featName]/actions/` directory.  
Of course, there may be exceptions, but be rational when deciding where to place your actions.